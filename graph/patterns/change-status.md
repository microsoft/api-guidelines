...
If the response triggers a long running operation, the response should contain a reference to query its status. The status on the resource shall stay the same until the long running operation is completed:
